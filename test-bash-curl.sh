...
echo "-----------------------------"
echo "Running: curl -s -H 'X-Test: Value' http://localhost:$PORT/test"
OUTPUT=$(curl -s -H "X-Test: Value" http://localhost:$PORT/test)
echo "Response: $OUTPUT"

if [[ "$OUTPUT" == *"1.1"* ]] && [[ "$OUTPUT" == *"/test"* ]]; then
...
echo "---------------------------"
echo "Running: curl -s --http2-prior-knowledge -H 'X-Test: Value' http://localhost:$PORT/api"
OUTPUT=$(curl -s --http2-prior-knowledge -H "X-Test: Value" http://localhost:$PORT/api)
echo "Response: $OUTPUT"

if [[ "$OUTPUT" == *"2.0"* ]] && [[ "$OUTPUT" == *"/api"* ]]; then
